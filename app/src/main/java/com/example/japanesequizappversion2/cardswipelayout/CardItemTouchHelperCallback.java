...
public class CardItemTouchHelperCallback<T> extends ItemTouchHelper.Callback {
    ...
    public void onSwiped(RecyclerView.ViewHolder viewHolder, int direction) {
        ...
        if (adapter instanceof DekiruGramaAdapter) {
            ((DekiruGramaAdapter) adapter).updateNo();
        } else if (adapter instanceof DekiruKenjiAdapter) {
            ((DekiruKenjiAdapter) adapter).updateNo();
        }
        ...
        if (mListener != null) {
            mListener.onSwiped(viewHolder, remove,
                    direction == ItemTouchHelper.LEFT ? CardConfig.SWIPED_LEFT
        ...
        if (actionState == ItemTouchHelper.ACTION_STATE_SWIPE) {
            ...
            if (childCount > CardConfig.DEFAULT_SHOW_ITEM) {
                for (int position = 1; position < childCount - 1; position++) {
                    ...
                    float v = 1 - index * CardConfig.DEFAULT_SCALE
                            + Math.abs(ratio) * CardConfig.DEFAULT_SCALE;
                    view.setScaleX(v);
                    view.setScaleY(v);
            ...
            if (mListener != null) {
                if (ratio != 0) {
                    mListener.onSwiping(viewHolder, ratio,
                            ratio < 0 ? CardConfig.SWIPING_LEFT : CardConfig.SWIPING_RIGHT);
                } else {
